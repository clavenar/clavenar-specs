...
| 6 | Enable HIL + identity receive-side mTLS in deployment: mint `service-simulator.{crt,key}` via `gen_certs.sh` (already wired for every other service), add the outbound-TLS helper to `warden-simulator/src/boot.rs` mirroring the console pattern, flip simulator's `WARDEN_HIL_URL` + `WARDEN_SIM_SVID_URL` + `WARDEN_SIM_ENROLL_IDENTITY_URL` schemes. Then set `WARDEN_HIL_TLS_DIR` + `WARDEN_IDENTITY_TLS_DIR` in compose/helm. E2E verify (`warden-e2e/run-stack-e2e.sh` extended), threat-model section trim, runbook for cert-pair rotation, spec lock. | `warden-simulator`, `warden-e2e/`, `warden-specs/TECH_SPEC.md` | Queued |
