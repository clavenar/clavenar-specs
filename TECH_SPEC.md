...
| 5 | Hil + ledger receive (dual-mode listener via `WebPkiClientVerifier::allow_unauthenticated()`: SPIFFE-gated routes for proxy/console/simulator, plain auth for browser approver + demo-session reads). Console-side outbound mTLS for ledger / hil / policy / identity. Identity-receive-path SVID validation. | `warden-hil`, `warden-ledger`, `warden-console`, `warden-identity`, `warden-proxy` (proxy→hil URL flip) | Queued |
