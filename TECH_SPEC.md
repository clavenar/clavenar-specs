...
- **`clavenar-chaos-monkey`** gains: `invalid_actor_token`, `unattested_binary`, `expired_grant`, `cross_tenant_unfederated`. All four must produce specific, predicted verdicts. Actor-token replay is covered only by the stateful identity contract test that successfully redeems an authorized token before reusing its JTI; a single unsigned request must not be described as replay coverage.
...
- **Optional Exec containment (WP-13.1)** — the official default and
  production profiles contain no Exec Deployment, Service, PVC, or
  NetworkPolicy, and production rejects `exec.enabled=true`. Evaluation-only
  opt-in requires workload TLS and NetworkPolicy. Proxy uses a dedicated HTTPS
  upstream client presenting its workload identity; Exec verifies the chain and
  requires the exact `spiffe://clavenar.local/service/proxy` caller before
  trusting `x-clavenar-agent-id`. Missing or malformed identity is rejected
  before dispatch. Plain health and readiness live on unpublished `:9002` in a
  separate Router with no MCP fallback; the ClusterIP Service and Proxy-only
  ingress policy expose only mutual-TLS `:9001`. The exact
  excluded-disabled contract is
  [`contracts/exec-surface-containment-v1.schema.json`](contracts/exec-surface-containment-v1.schema.json)
  with its canonical
  [fixture](contracts/exec-surface-containment-v1.fixture.json).
- **Structured Exec isolation (WP-13.2)** — evaluation-only Exec exposes
  `execute_command`, never `bash`; the removed `cmd` string and `bash -c` /
  `sh -c` paths are invalid. A call selects one policy command identifier and
  supplies exactly the ordered named argument slots plus an exact environment
  allowlist. Exec resolves the policy's absolute executable directly, clears
  inherited environment, applies fixed environment values, and rejects unknown,
  missing, duplicate, extra, reordered, wrong-kind, or overlong input before
  process creation. The initial allowlist contains only `render_text` at
  `/usr/bin/printf`, with one fixed format slot, one bounded text slot, fixed
  locale, and optional `TERM=dumb`; metacharacters are ordinary argv bytes.
  The evaluation chart requires a digest-pinned image and mounts the immutable
  policy read-only. UID/GID 65532, read-only root, bounded memory-backed scratch,
  RuntimeDefault seccomp, no privilege escalation, dropped capabilities, and
  default-deny egress provide the OS/container boundary. Only cluster DNS and
  the exact in-cluster fallback peer are admitted; production absence and
  opt-in rejection remain unchanged. Filesystem rooting, process-tree/resource
  ceilings, and DNS/redirect validation remain WP-13.3 through WP-13.5. The
  strict contract is
  [`contracts/structured-execution-v1.schema.json`](contracts/structured-execution-v1.schema.json)
  with its canonical
  [fixture](contracts/structured-execution-v1.fixture.json).
- **Hard Exec ceilings (WP-13.3)** — every limit is compiled and server-owned;
  callers and runtime environment cannot select or weaken it. JSON-RPC bodies
  stop at 4 MiB. Each command receives a new process group, a 30-second wall
  clock, 10 CPU seconds, 256 MiB address space, 16 processes, 8 MiB file-size
  limit, and 64 open files. Timeout kills the complete process group and reaps
  the direct child. Stdout and stderr drain concurrently, retain at most
  256 KiB each, and append an exact truncation marker while discarding later
  bytes. File-tool content is capped at 8 MiB, directory listings at 4,096
  entries, search output at 1 MiB, and fetch request/response bodies at 1 MiB.
  `CLAVENAR_EXEC_TIMEOUT_SECS` is invalid rather than an override. Default and
  production remain resource-absent and reject opt-in. The strict contract is
...
- **Hosted Lite safety (WP-13.6)** — the developer-laptop posture remains the
  default, but every internet-hosted template explicitly selects
  `CLAVENAR_LITE_DEPLOYMENT_PROFILE=hosted`. That profile refuses startup
  unless agent and operator authentication are both present, unambiguous,
  at least 32 bytes, and disjoint; mode is `enforce`; verbose verdicts are
  off; and a positive per-tenant-agent token bucket stays within the fixed
  0.1–100 QPS and 1–200 burst bounds. The SQLite path must be an absolute
  non-temporary file beneath the persistent `/data` mount. Hosted templates
  keep one machine running so hash-chain, pending, decision, and durable
  server-execution state survive restart. The only hosted upstream adapter is
  explicit `mcp-jsonrpc-v1`: it requires a non-placeholder HTTPS target,
  sends and receives JSON with 1 MiB request/response caps and a 30-second
  whole-operation ceiling, accepts JSON-RPC 2.0 only, and binds the response
  ID exactly to the request without automatic effect retry. Empty callback
  allowlists continue to deny and Exec remains absent from production. The
  strict contract is
  [`contracts/hosted-lite-safety-v1.schema.json`](contracts/hosted-lite-safety-v1.schema.json)
  with its canonical
  [fixture](contracts/hosted-lite-safety-v1.fixture.json).
...
- **Explicit client migration (WP-06.11)** — Proxy 0.5.0 and Lite 0.9.0 reject every effect-capable unselected `POST /mcp` request with HTTP 426 `client_contract_required` and `executable:false` before authentication-side mutation, rate limiting, policy, HIL, Ledger, receipt, or upstream work. Unselected MCP handshake, notification, ping, and catalog methods remain compatible and tool-effect-free. An executing caller must explicitly pair `clavenar.server-execution/v1` with a canonical pre-network idempotency ID; an inspecting SDK must explicitly pair `clavenar.decision/v1` with that ID. Legacy execution headers, partial pairs, mixed selectors, and unknown versions remain zero-effect invalid requests. The public [migration guide](docs/SDK_MIGRATION.md) and exact [`clavenar.client-migration/v1` fixture](contracts/client-migration-v1.fixture.json) define minimum safe versions, staged client-first rollout and gateway-first rollback, pending resume, uncertain reconciliation, and receipt delivery for every maintained language.
...
**Module status:** **shipped (manifest v8; tenant-bound export in v1.216.0).** Lives in `clavenar-ledger` + `clavenar-identity` (`POST /sign/blob`) + `clavenar-sdk` + `clavenarctl` + `clavenar-console`. No chain-version change.
...
The signature commits to `sha256(canonical_manifest_with_signature_blanked_to_null)` so `tenant`, `verified_chain`, `technical_documentation`, `parquet_pointers`, `compliance_register`, `anchors`, `annex_iv`, and `post_market_monitoring_plan` are signed transitively — tampering with any of them breaks both signature verification and a cheap recompute. v1 was the unsigned shape (slice 1); v2 added the `signature` envelope (slice 2); v3 added the optional `technical_documentation` and `parquet_pointers` blocks (slice 3); **v4** added the optional `compliance_register` block; **v5** added the optional `anchors` block; **v6** added the optional `annex_iv` and `post_market_monitoring_plan` blocks; **v7** adds the required `verified_chain` binding governed by `contracts/regulatory-bundle-signing-v1.schema.json`; **v8** requires and signs the authenticated tenant partition.
...
Lives under the top-level `regulatory` verb. `--tenant` resolves through the
normal flag → `CLAVENAR_TENANT` → `default_tenant` chain, and the secure
transport profile attaches the tenant-bound operator credential. The SDK
refuses a regulatory request without an explicit tenant; Ledger requires the
predicate, reads only matching rows, rejects any out-of-partition row before
signing, and commits the tenant in manifest v8. Console offers the same
download from `/exports`, deriving the tenant from the verified operator
session rather than browser input and emitting a no-store audited response.
...
The approved procedure is export-before-erasure and is fail closed:

1. Authenticate an Admin for exactly one tenant and create the durable
   offboarding operation before owner effects.
2. Fence authority, credentials, caches, Policy, and HIL in the declared saga
   order.
3. Produce and retain one signed manifest-v8 final export whose `tenant`
   matches the operation. Export failure leaves the saga retryable and blocks
   tombstoning.
4. Tombstone live tenant reads without rewriting immutable chain bytes. Record
   minimized deletion receipts; do not claim physical chain erasure.
5. Advance the HMAC-authenticated retained-backup erasure disposition. Every
   restore applies that disposition before readiness, while an active legal
   hold defers destructive deletion and remains auditable.
6. Verify same-name cross-tenant reads, exports, and restored state remain
   unaffected.
...
| POST | `/export/regulatory?tenant=…&…&include_compliance=true[&annex_iv=true][&include_post_market_plan=true]` (clavenar-ledger) | optional `text/markdown` | `.tar.gz` with embedded `compliance_register.json` (+ optional `annex_iv_documentation.json` / `post_market_monitoring_plan.json`), manifest v8 |
...
### Transactional producer custody

Every reachable governed effect appears exactly once in the v1 effect
inventory. Existing SDK, selected Proxy/Lite server execution, Policy
lifecycle, and Identity agent-lifecycle durable stores remain required and are
continuously source-certified. Unselected effect-capable Proxy and Lite calls
return the typed non-executable client-migration response before policy,
forensic publication, or upstream dispatch.

HIL owns one append-only `forensic_outbox` in the same SQLite database as
`pending_requests`. Pending create, single decision, quorum vote/commit,
bounded modification, and TTL expiry insert `operation.intent`, mutate the
pending row, insert the causally linked `transition.committed`, and commit one
transaction. A failed insert or terminal append rolls back the transition;
idempotent create/approval retries create no second event.

Identity owns a separate append-only `forensic_outbox` in its authority
database. SVID, delegation-grant, actor-token, action-signature, and detached
blob-signature paths commit `operation.intent` before calling the CA or signer.
They commit `credential.released` before returning credential bytes; if that
terminal commit fails, the generated bytes are not returned. A signer/CA
failure may retain an intent without a success stage so later reconciliation
can classify the interruption without inventing a release.

### Transactional Ledger uniqueness

The Ledger accepts the strict envelope only when the complete top-level field
set, contract vocabulary, canonical JSON bytes, canonical UUIDs, provenance,
workload identity, and recomputed payload commitment validate. It computes the
envelope commitment over the exact received bytes and treats
`(producer.service, event_id, stage)` as the immutable consumer identity.

SQLite and PostgreSQL create an append-only `forensic_stages` claim table
without modifying historical rows or hashes. The claim stores both commitments,
the exact envelope, and a snapshot of the linked chain entry. In one database
transaction the Ledger either appends one row plus one claim, returns the
identical retained entry, or rejects a changed-byte tuple conflict. The compact
`policy_decision.forensic_identity` object commits the tuple and both digests
through the existing chain-version selection, so no historical hash shape is
rewritten. A failed row or claim write rolls back both.

Created and retained messages receive a normal JetStream acknowledgement.
Malformed envelopes and tuple conflicts append nothing and receive an explicit
terminal consumer disposition with separate high-signal counters; retryable
storage failures remain unacknowledged. The retained entry snapshot keeps the
claim authoritative after the hot row moves to cold storage. Legacy
`LogRequest` messages retain their compatibility path and do not receive
fabricated strict identities.

Both tables retain canonical envelope bytes and their SHA-256, exact producer
...
**Module status:** **shipped in v1.221.0; the current weekly recovery-point
cadence explicitly does not meet the critical-state RPO.**
...
new cluster, anonymously pulls chart `0.36.11`, installs its packaged bundled
values plus `clavenar-images-1.245.16.yaml`, waits for Jobs and workloads, and
...
**Module status:** **release acceptance in progress for v1.245.16.**
